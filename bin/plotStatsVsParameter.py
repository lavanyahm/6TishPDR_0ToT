...
CONFINT         = 0.95
START_CYCLE     = 1 # statistics (reliability, packet loss, latency) are collected after this cycle
...
def plot_flowpdr(elemName):
    ...
    for dataSetDir in dataSetDirs: 
        ...
        for dir in os.listdir(dataSetDir):
            ...
            for infilename in glob.glob(os.path.join(dataSetDir, dir,'*.dat')):
                ...
                elem, flowpdrEachElem = calcflowpdr(
                    dir                = os.path.join(dataSetDir, dir),
                    infilename         = os.path.basename(infilename),
                    elemName           = elemName
                )
                
                flowpdrEachSet[elem]    = flowpdrEachElem
                                
        flowpdr[dataSetDir] = flowpdrEachSet
    ...
    outfilename    = 'output_flowpdr_{}'.format(elemName)
    ...
    for dataSetDir in dataSetDirs:
        bar_flowpdrEachSet = {}
        for elem, flowpdrEachElem in flowpdr[dataSetDir].items():
            bar_flowpdrEachSet[elem] = [1.0-v for v in flowpdrEachElem]
        bar_flowpdr[dataSetDir] = bar_flowpdrEachSet
...
def calcReliability(dir,infilename,elemName):
    ...
    with open(infilepath,'r') as f:
		...
		genNoZero = float(totalGenerated+initTxQueueFill-txQueueFill)
		if genNoZero==0:
			genNoZero=1

                reliabilities  += [float(totalReaches)/genNoZero]
...
def calcLatency(dir,infilename,elemName):
    ...
    with open(infilepath,'r') as f:
        for line in f:
            ...
            if cycle == numCyclesPerRun-1:
		if sumappReachesDagroot==0:
			sumappReachesDagroot =1
