...
def plot_vs_time_single_threshold(plotData,ymin=None,ymax=None,ylabel=None,filename=None,doPlot=True):
    prettyp   = False
...
def plot_latency_vs_time(dataBins):
    ...
    plot_vs_time_single_threshold(
        plotData = plotData,
        ymin     = 0,
        ymax     = 8,
        ylabel   = 'end-to-end latency (s)',
        filename = 'latency_vs_time_threshold',
    )

def gather_pdr_data(dataBins):
    ...
    plotData  = {}
    for ((otfThreshold,pkPeriod),filepaths) in dataBins.items():
        plotData[(otfThreshold,pkPeriod)] = gatherPerCycleData(filepaths,'aveFlowPdr')
...
def plot_flowPdr_vs_time(dataBins):
    
    plotData  = gather_pdr_data(dataBins)
    
    plot_vs_time(
        plotData = plotData,
        ymin     = 0,
        ymax     = 8,
        ylabel   = 'end-to-end pdr (s)',
        filename = 'flowPdr_vs_time',
    )
...
def plot_reliability_vs_time(dataBins):
    ...
    for ((otfThreshold,pkPeriod),filepaths) in dataBins.items():
        ...
        appGeneratedData[(otfThreshold,pkPeriod)]=gatherPerCycleData(filepaths,'appGenerated')
        appReachedData[  (otfThreshold,pkPeriod)]=gatherPerCycleData(filepaths,'appReachesDagroot')
   ...
   #         q = float(txQueueFillData[otfThreshold_pkPeriod][cpuID_runNum])
    ...
    # calculate the end-to-end reliability per (otfThreshold,pkPeriod)
    for ((otfThreshold_pkPeriod),perCycleData) in reliabilityData.items():
    		vals = reliabilityData[otfThreshold_pkPeriod].values()
        	(m,confint) = calcMeanConfInt(vals)
        	reliabilityData[cycle] = {
            		'mean':      m,
            		'confint':   confint,
        	}
    ...
    # arrange to be plotted
    for ((otfThreshold,pkPeriod),perCycleData) in reliabilityData.items():
        x     = sorted(perCycleData.keys())
        y     = [perCycleData[i]['mean']    for i in x]
        yerr  = [perCycleData[i]['confint'] for i in x]
        assert len(x)==len(y)==len(yerr)
