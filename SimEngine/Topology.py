...
class Topology(object):
    ...
    def __init__(self,motes,moteLocations):
        print"[Topology]:: Topology _init_"
        # store params
        self.motes           = motes
        self.moteLocations    =moteLocations      
        # local variables
	...
	self.sourceNode =None
        ...
        for mote in self.motes:
          for MoteLocation in self.moteLocations:
            if (mote.id==0 and int(MoteLocation[0]) ==0) :
	      
                mote.role_setDagRoot()
                dagRoot = mote
                x = float(MoteLocation[1])
                y = float(MoteLocation[2])
        	dagRoot.setLocation( x, y )
                setDagRoot =True
                break  
          if setDagRoot == True:
	      break	 
        ...
        for mote in self.motes:
            ...
            while not connected:
                # pick a random location
            	 for MoteId in self.moteLocations:
              	    if int(MoteId[0]) == mote.id:    
                       mote.setLocation(
                       x = float(MoteId[1]),
                       y = float(MoteId[2])
                       )
        ...
        forMoteId =0
        for mote in self.motes:
	      distance	 = self._computeDistance(mote,dagRoot)
	      if distance > forDistance:
		 forDistance = distance  
                 forMoteId = mote.id

        for mote in self.motes:
		if mote.id == forMoteId:
                   self.sourceNode = mote
                   mote.role_setSourceNode()
