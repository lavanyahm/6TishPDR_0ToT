...
class Propagation(object):
    ...
    #==== Markov Propogation Model Values
	...
	#L0: 1->0 starts in state 0, L1: 2->1 starts in state 1, L2:3->2 starts in state 2
	self.cur_state		       = [0, 1, 2]  
        
        # schedule propagation task
        self._schedule_propagate()

	if self.settings.enableMarkovPropagation:
	  # schedule changing channel condition
	  for link in self.cur_state:
	     self._schedule_changeChannelCond(link)
        ...
        with self.dataLock:
            ...
            for transmission in self.transmissions:
                ...
                while i<len(self.receivers):
				    ...
				    failure = self._getFailureVal(transmission['dmac'].id)
				    #print transmission['smac'].id, '>', transmission['dmac'].id, 'channel: ', transmission['channel'], 'pdr: ', pdr, 'failure: ', failure 
                                    if pdr>=failure:
    ...
    def _schedule_changeChannelCond(self, link):
        with self.dataLock:
	    delay = random.uniform(0.1,1.0)
            self.engine.scheduleIn(
                #delay      = self.changeChannelCondPeriod*(0.9+0.2*random.random()),
                delay       = delay,
                cb          = functools.partial(self._changeChannelCond, link),
                uniqueTag   = (None,'changeChannelCond', link),
                priority    = 6,
            )
    ...
    def _getFailureVal(self, linkId):
        ''' returns the failure value which represents the channel condition'''

        if not self.settings.enableMarkovPropagation:
		return random.random()

	#Use markov propagation model
	state = self.cur_state[linkId];
	
	if state == 0:
		failure = self.MARKOV_S0
	elif state == 1:
		failure = self.MARKOV_S1
	elif state == 2:
		failure = self.MARKOV_S2
	else:
		print 'Shouldnt be here. In wrong state'
		
        return failure

    def _changeChannelCond(self, link):
	''' Use Markov model to get failure value '''

	random_no = random.random()
	cur_state = self.cur_state[link]
	...
	if cur_state == 0:
		...
		if random_no <= 0.3:
			new_state = 1
		elif random_no > 0.3 and random_no <= 0.7:
			new_state = 2
		else:
			new_state = 0
	elif cur_state == 2:
	...
	print 'Link ', link, 'moving from ', cur_state, '->', new_state
	self.cur_state[link] = new_state
