...
class Mote(object):
    ...
    #IISc 
    DELAY_CONSTRAINT_F1                 = 30	 
    DELAY_CONSTRAINT_F2                 = 40		 
	...
	self.sourceNode		       = 0
        #otf
        self.numDefaultCells           = self.settings.numDefaultCells       
        self.isDelayConstraint          = self.settings.isDelayConstraint

 	#File based Cell Allocation
	self.enableCellAllocation      = self.settings.enableCellAllocation
	...
	self.deadlineFlow1            =self.settings.deadlineFlow1
	self.deadlineFlow2            =self.settings.deadlineFlow2
	self.deadlineFlow3            =self.settings.deadlineFlow3
	self.deadlineFlow4            =self.settings.deadlineFlow4

#Propagation Model
        self.enableMarkovPropagation   = self.settings.enableMarkovPropagation

        for i  in range (1, self.numFlows+1):
            self.txQueues[i] = [] 
        # stats
        #import pdb;pdb.set_trace()
        self._stats_resetMoteStats(True)
        self._stats_resetQueueStats()
        self._stats_resetLatencyStats()
        self._stats_resetHopsStats()
        self._stats_resetRadioStats()

	#Scheduling Pce update from here. As in SimPce is init only for first time
	# and remaining runs it is simply returned. 
	#PCE - Schedule updating PDR periodically 
	self.pce._schedule_updatePDR() 
    ...
    def role_setDagRoot(self):
        ...
        self.packetLatencies  = [ [] for i in range (1, self.numFlows + 1)] # in slots
        self.packetHops           = []

    def role_setSourceNode(self):
        self.sourceNode           = True
        print "Mote.Py SourceNode",self.id
    
    #===== application
    
    def _app_schedule_multipleFlowsAtAsn(self,flowId,startAsn=0,firstPacket=False):
        '''
        create an event that is inserted into the simulator engine to send the data according to the traffic
        '''
        
	self._log(self.INFO, "[app], _app_schedule_multipleFlowsAtAsn: ASN {0}, Flow ID: {1}", (startAsn, flowId),)       
        if not firstPacket:
            ...
            self.engine.scheduleIn(
                delay            = delay,
                cb               = functools.partial(self._app_action_multipleFlowsAtAsn,flowId),
                uniqueTag        = (self.id, '_app_action_multipleFlowsAtAsn', flowId),
                priority         = 2,
            )
        else:
            # schedule
    ...
    def _app_action_sendSinglePacket(self):
        ...
        # enqueue data
        self._app_action_enqueueData(flowId)
        
        # schedule next _app_action_sendSinglePacket
        self._app_schedule_sendSinglePacket()
    
    def _app_action_multipleFlowsAtAsn(self, flowId):
        ''' actual send data function. Send Multiple flows '''
       
        # enqueue data
        self._app_action_enqueueData(flowId)
        
        # schedule next _app_action_sendSinglePacket
        self._app_schedule_multipleFlowsAtAsn(flowId)
    
    def _app_action_enqueueData(self,flowId):
        ''' enqueue data packet into stack '''
        
        # only start sending data if I have some TX cells
        if self.getTxCells():
            if flowId ==  1:
		packetDeadline =  self.engine.getAsn()+ self.deadlineFlow1
            elif flowId == 2 :
		packetDeadline =  self.engine.getAsn()+ self.deadlineFlow2
                        
            elif flowId == 3 :
		packetDeadline =  self.engine.getAsn()+ self.deadlineFlow3

            else  :
		packetDeadline =  self.engine.getAsn()+ self.deadlineFlow4
            # create new packet
            newPacket = {
                'asn':            self.engine.getAsn(),
                'type':           self.APP_TYPE_DATA,
                'payload':        [self.id,self.engine.getAsn(),1, packetDeadline,flowId], # the payload is used for latency and number of hops calculation
                'retriesLeft':    self.TSCH_MAXTXRETRIES
            }
            self._log(self.INFO, "[app], _app_schedule_multipleFlowsAtAsn: ASN {0}, packet: {1}", (self.engine.getAsn(),newPacket),) 
            # update mote stats
            #IISc
	    #import pdb;pdb.set_trace()
            appGenerated = "appGenerated_F" +str(flowId)
            self._stats_incrementMoteStats(appGenerated)
            self.pce.stats_updatePktStatus(appGenerated)
    ...
    def _rpl_action_sendDIO(self):
        ...
        self._log(
            self.INFO,
            "[rpl]: My rank is {0} and dagRank is {1}",
            (self.rank,self.dagRank),
        )
        with self.dataLock:
    ...
    def _rpl_housekeeping(self):
        with self.dataLock:
            ...
            self._log(
                self.INFO,
                "[rpl-housekeeping]: My rank is {0} and dagRank is {1}",
                (self.rank,self.dagRank),
            )
            ...
            for (parent,portion) in sorted_parent_portion:
                ...
                if nowCells==0 or nowCells<reqCells:
                    ...
                    if reqCells>0:
                        ...
                        #numCellsToAdd = 1
                        #IISc_Cell  
                        numCellsToAdd = self.numDefaultCells
    ...
    def _sixtop_txhousekeeping_per_neighbor(self,neighbor):
		...
		if self.enableCellAllocation == True: 
			#Use the cell with same ts, change only the channel for the given worst_ts
                	self._sixtop_cell_change_channel_request(neighbor,worst_ts)
		else:
    ...
    def _sixtop_rxhousekeeping_per_neighbor(self,neighbor):
	    ...
	    if self.enableCellAllocation == True: 
	    	#Use the cell with same ts, change only the channel for the given worst_ts
            	self._sixtop_cell_change_channel_request(neighbor,ts,dir=self.DIR_RX)
	    else:
            	# relocate: add new first
    ...
    def _sixtop_cell_change_channel_request(self,neighbor,ts,dir=DIR_TX):
	''' Reallocate the cell by just changing the channel for a given ts'''
	with self.dataLock:
	 newChan = neighbor._sixtop_cell_change_channel_response(self,ts,dir)

	 with self.dataLock:
	   for key,cell in self.schedule.iteritems():
             if key == ts and dir == cell['dir']:
		print 'Cur channel is: ', cell['ch'], 'New channel is:', newChan
		cell['ch'] = newChan 
		self.schedule[key] = cell
		return

    def _sixtop_cell_change_channel_response(self,neighbor,ts,dirNeighbor):
	''' Reallocate the cell by just changing the channel for a given ts'''
	...
	with self.dataLock:
	   ...
	   for key,cell in self.schedule.iteritems():
             if key == ts and dir == cell['dir']:
		curChan = cell['ch']
		newChan = random.choice(range(0, curChan) + range(curChan+1, self.settings.numChans - 1))
				 
		cell['ch'] = newChan 
		self.schedule[key] = cell
		print 'Cur channel is: ', curChan, 'New channel is:', newChan
		return newChan

	print 'Error:TS not found'
    ...
    def _sixtop_cell_reservation_request(self,neighbor,numCells,dir=DIR_TX):
        ...
        with self.dataLock:
            self._stats_incrementMoteStats('sixtop_cell_reservation_request')
    ...
    def _sixtop_cell_reservation_response(self,neighbor,numCells,dirNeighbor):
        ...
        with self.dataLock:
            self._stats_incrementMoteStats('sixtop_cell_reservation_response')
            # set direction of cells
            if dirNeighbor == self.DIR_TX:
                dir = self.DIR_RX
            else:
	    ...
	    if self.enableCellAllocation == True: 
                #PCE
	    	numCells, timeSlots = self.pce.get_slots(self.id, neighbor.id, dir)
	    	cells = dict([ (ts, random.randint(0, self.settings.numChans-1)) for ts in timeSlots[:numCells]])
	    else:
    ...
    def _tsch_enqueue(self,packet):
        ...
        elif self.TSCH_QUEUE_SIZE == self._getlenQueue(packet['payload'][4]): 
            # my TX queue is full
    ...
    def _tsch_action_activeCell(self):
        ...
        with self.dataLock:
            ...
            if  cell['dir']==self.DIR_RX:
                ...
                self.pktToSend = self._getPacket()
                self._log(self.INFO, "[radio_txDone]  ACK Recived at  ASN {0}, Packet {1}  ", (self.engine.getAsn(), self.pktToSend),) 
    ...
    def radio_txDone(self,isACKed,isNACKed):
        ...
        asn   = self.engine.getAsn()
        ts    = asn%self.settings.slotframeLength
        self._stats_incrementMoteStats('pktSent')
		...
		for k,v in self.txQueues.iteritems():
		    if self.pktToSend in v:
                	self.txQueues[k].remove(self.pktToSend)
		...
		for k,v in self.txQueues.iteritems():
		    if self.pktToSend in v:
		       listIndex = v.index(self.pktToSend)
		       qIndex = k
		       if v[listIndex]['retriesLeft'] > 0:
			  self.txQueues[qIndex][listIndex]['retriesLeft'] -= 1
			  break

                # drop packet if retried too many time
	        if self.txQueues[qIndex][listIndex]['retriesLeft'] == 0:
                    #IISc Commented  LHM
                    #if  len(self.txQueues[qIndex]) == self.TSCH_QUEUE_SIZE:
                        # update mote stats
			droppedMacRetries = "droppedMacRetries_F" + str(self.pktToSend['payload'][4])
                        self._stats_incrementMoteStats(droppedMacRetries)
                        self._log(self.INFO, "[radio_txDone] isNACKed  droppedMacRetries at  ASN {0}, Packet {1}", (self.engine.getAsn(),self.pktToSend),) 
                        
                        # remove packet from queue
                        self.txQueues[qIndex].remove(self.pktToSend)
		...
		for k,v in self.txQueues.iteritems():
		    if self.pktToSend in v:
		       ...
		       qIndex = k
		       if v[listIndex]['retriesLeft'] > 0:
			  self.txQueues[k][listIndex]['retriesLeft'] -= 1
			  break
...
#                if self.txQueue[i]['retriesLeft'] == 0:
    ...
    def radio_rxDone(self,type=None,smac=None,dmac=None,payload=None):
        '''end of RX radio activity'''
      	self._stats_incrementMoteStats('pktRecived')
        ...
        with self.dataLock:
            ...
            if smac:
        	    ...
        	    self._log(self.INFO, "[radio_rxDone] appReachesDagroot @ ASN {0}, Packet {1}", ( self.engine.getAsn(), payload),)           # update mote stats
                     
                    #self._stats_incrementMoteStats('appReachesDagroot')
                    #IISc
                    appReachesDagroot = "appReachesDagroot_F" + str(payload[4])
                    self._stats_incrementMoteStats(appReachesDagroot)
                    #PCE
                    self.pce.stats_updatePktStatus(appReachesDagroot)
                    # calculate end-to-end latency
                    self._stats_logLatencyStat(asn-payload[1], payload[4])
                    ...
                    self._log(self.INFO, "[radio_rxDone] appReached_ Mote {0} ASN {1}, Packet{2}  ", (self.id,self.engine.getAsn(), payload),) 
                  
                       #IISc : Check if delay is exced
                    if self.isDelayConstraint == True and  payload[3] <=  self.engine.getAsn():
                          droppedDelayExceed = "droppedDelayExceed_F" +str (payload[4])
                          self._stats_incrementMoteStats(droppedDelayExceed)
                       	  self._log(self.INFO, "[radio_txDone]  app Recived  droppedDelayExceed at  ASN {0}, Packet {1} ", (self.engine.getAsn(), payload),)
                          (isACKed, isNACKed) = (True, False)
                	    ...
                	    if isEnqueued:
                        	...
                        	# update mote stats
                                #IISc 
                                appRelayed = "appRelayed_F"+ str(payload[4])
	...
	#IISc- Seema: Calculate drift only w.r.t your parent. If routing loops
	#exist, then the following while loop will loop forever. Hence, commenting
	...
	'''
		...
		print 'child: ', child.id, 'parent: ', parent.id
        '''
        return offset
	...
	#self._setSrc()       
        ...
        # app
	if not self.dagRoot and self.sourceNode:
            if self.settings.numPacketsBurst!=None and self.settings.burstTimestamp!=None:
                ...
                startAsnList = [100,100,100,100]  
                for i in range(self.numFlows):
                    startAsn = startAsnList[i] 
                    flowId = i + 1
                    self._app_schedule_multipleFlowsAtAsn(flowId, startAsn , firstPacket=True)
    ...
    def getMoteStats(self):
        ...
        with self.dataLock:
            ...
            returnVal['txQueueFill']        = len(self.txQueue) # TODO : Get Length of all Queues
            returnVal['chargeConsumed']     = self.chargeConsumed
            returnVal['numTx']              = sum([cell['numTx'] for (_,cell) in self.schedule.items()])

            for i in range (1, self.numFlows+1):
	        aveLatencyName   = "aveLatency_F" + str(i)
		returnVal[aveLatencyName] = self._stats_getAveLatency(i)

		
        
        # reset the statistics
        #import pdb;pdb.set_trace()
        self._stats_resetMoteStats(False)
        self._stats_resetQueueStats()
        self._stats_resetLatencyStats()
        self._stats_resetHopsStats()
        self._stats_resetRadioStats()
    ...
    def _stats_resetMoteStats(self,isInitialize):
        with self.dataLock: 
              ...
              for i in range (1, self.numFlows+1):
                 ...
                 appRelayed       = "appRelayed_F"+ str(i)
                 appReachesDagroot = "appReachesDagroot_F" +str(i)         
		 droppedMacRetries = "droppedMacRetries_F" + str(i)
                 
	    	 if  isInitialize  == False:		 
                     temp[appGenerated ]  = self.motestats[appGenerated]  # number of packets app layer generated
                     temp[appRelayed]    = self.motestats[appRelayed]   # number of packets relayed
                     temp[appReachesDagroot]= self.motestats[appReachesDagroot]   # number of packets received at the DAGroot
                     temp[droppedMacRetries] =self.motestats[droppedMacRetries]   # packets dropped because more than TSCH_MAXTXRETRIES retries
 
                     if self.isDelayConstraint == True :
                     	droppedDelayExceed = "droppedDelayExceed_F" + str(i) 
                     	temp[droppedDelayExceed] =  self.motestats[droppedDelayExceed]  #packets dropped because exceed delay Contraints
		 else :
			
                     temp[appGenerated ]  = 0  # number of packets app layer generated
                     temp[appRelayed]    = 0    # number of packets relayed
                     temp[appReachesDagroot]= 0   # number of packets received at the DAGroot
                     temp[droppedMacRetries] = 0   # packets dropped because more than TSCH_MAXTXRETRIES retries
 
                     if self.isDelayConstraint == True :
                     	droppedDelayExceed = "droppedDelayExceed_F" + str(i) 
                     	temp[droppedDelayExceed] = 0 #packets dropped because exceed delay Contraints
              ...
              self.motestats = {
                ...
                'sixtop_cell_reservation_request':    0, #number of cell reservation  request packets sent
                'sixtop_cell_reservation_response':    0, #number of cell reservation  response  packets sent
                 
                #PCE
		'updatePDRStatus'                 :0,#number of time  debt updated by PDR 
               # 'updatePktStatus'                 :0,#number of time PCE is updated with Pkt  generation

                # tsch
                #'droppedMacRetries'              :0,# packets dropped because more than TSCH_MAXTXRETRIES MAC retries
                #TxRxDone
                'pktSent'                         :0, #No of Packet Transmited by Mote
                'pktRecived'                      :0 #No of Packet Recived  by Mote

            }
            #IISc   
            #Number of Flows
              print"-----\n Bf:  isInitialize {0}   moteStats {1}  temp {2}   ".format(isInitialize,self.motestats,temp) 
              self.motestats.update(temp)  
              print" Af:  isInitialize {0}   moteStats {1}  temp {2}---   ".format(isInitialize,self.motestats,temp) 
    ...
    def _stats_logLatencyStat(self,latency, flowId):
        with self.dataLock:
            self.packetLatencies[flowId-1] += [latency]
    
    def _stats_getAveLatency(self, flowId):
        with self.dataLock:
            d = self.packetLatencies[flowId-1]
...
#IISc 
    def  _getlenQueue(self,qId):
         with self.dataLock:
              return len(self.txQueues[qId])
    def _enqueInTxQue(self,packet):
        with self.dataLock:
             qId = packet['payload'][4]
             self.txQueues[qId] += [packet]
             #self._log(self.INFO, "[tsch] len q1: {0}, len q2: {1}", ( len(self.txQueues[1]), len(self.txQueues[2])),)

  # To  proritize the queque
    def _getPacket(self):
        self.pktToSend = None 
        PDRDebtList    =  self.pce.getPDRDebt()
        while self.pktToSend == None and PDRDebtList:
                qId            = self._getHighestDebtQueue(PDRDebtList)  
                self._log(self.INFO, "[tsch] debts: {0}, qid: {1}", ( PDRDebtList, qId),)
		self.pktToSend = self._getPacketFromQueue(qId)
                if self.pktToSend == None :
                   del PDRDebtList[qId]
        return self.pktToSend

    #Get qid  of the Flow which  has highest debt
    def _getHighestDebtQueue(self,PDRDebtList):
       with self.dataLock:
	...
	with self.dataLock:
             ...
             if self.txQueues[qId]:
                    self.pktToSend = self.txQueues[qId][0]
                    if self.isDelayConstraint == True:
                       while self.txQueues[qId]:
                            self.pktToSend = self.txQueues[qId][0]
        	            if self.pktToSend['payload'][3] < self.engine.getAsn():
                               droppedDelayExceed = "droppedDelayExceed_F" +str(self.pktToSend['payload'][4])
                               self._stats_incrementMoteStats(droppedDelayExceed)
                               self._log(self.INFO, "[tsch] Mote:{0}  droppedDelayExceed at  ASN {1}, Packet {2} ", ( self.id,self.engine.getAsn(), self.pktToSend),)
                	       self.txQueues[qId].remove(self.pktToSend)
			       self.pktToSend = None
                    	    else :
                               break
